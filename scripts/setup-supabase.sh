...
# Script para configurar Supabase - Sistema Legal
# Este script crea todas las tablas, funciones, políticas RLS y triggers
...
read -p "📝 Ingresa tu Project ID de Supabase: " PROJECT_ID
read -p "📝 Ingresa tu Project URL (https://xxxxx.supabase.co): " PROJECT_URL
read -p "📝 Ingresa tu Service Role Key (eyJ...): " SERVICE_ROLE_KEY
...
supabase link --project-ref "$PROJECT_ID" || {
    echo "❌ Error al conectar con Supabase. Verifica tus credenciales."
    exit 1
}
...
# Crear archivo SQL temporal con todas las migraciones
SQL_FILE=$(mktemp)
cat > "$SQL_FILE" << 'EOF'
-- ============================================
...
# Ejecutar SQL usando psql o supabase db execute
if command -v psql &> /dev/null; then
    # Usar psql directamente
    PGPASSWORD="$SERVICE_ROLE_KEY" psql "$PROJECT_URL" -f "$SQL_FILE" || {
        echo "❌ Error al ejecutar SQL. Intentando con Supabase CLI..."
        supabase db execute --file "$SQL_FILE" || {
            echo "❌ Error al ejecutar migraciones."
            echo "   Ejecuta el SQL manualmente desde el SQL Editor en Supabase Dashboard"
            exit 1
        }
    }
else
...
echo "✅ Migraciones SQL ejecutadas correctamente"
echo ""
...
echo ""
echo "📋 Próximos pasos:"
echo "   1. Desplegar Edge Functions (ejecuta: ./scripts/deploy-functions.sh)"
echo "   2. Crear usuario admin (ver documentación)"
echo "   3. Configurar webhooks N8N en el panel de administración"
echo ""
